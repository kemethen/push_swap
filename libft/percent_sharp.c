...
/*   Updated: 2019/05/29 18:24:36 by kemethen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
size_t	percent_sharp_l(const char *str, va_list ap, t_var *v)
{
	...
	if (str[v->i + 1] == 'l' && str[v->i + 2] == 'f')
		percent_float(va_arg(ap, double), v, 'a');
	if (str[v->i + 1] == 'L' && str[v->i + 2] == 'f')
		percent_lfloat(va_arg(ap, long double), v, 'a');
	return (v->i + 3);
}
